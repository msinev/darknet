...
class imageCropLoader: imageSource  {
    const boost::filesystem::path abs;
public:
    imageCropLoader(const boost::filesystem::path &path, const networkImageInput &size) :abs(path), w(size.getWidth()), h(size.getHeight()) {}

    int w, h;
    image  getImage() override {
        image im = load_image_color((char*)abs.c_str(), 0, 0);
        if(im.h!=h || im.w!=w) {
            auto rc = center_crop_image(im, w, h);
            free_image(im);
            im=rc;
        }
        return im;
